...
{
    internal class NeteaseMusicSearchModule : SearchModule
    {
        public NeteaseMusicSearchModule()
        {
            SetInfo("网易云", "OLC", "2565128043@qq.com", NeteaseMusicApi.Version, "简单的修复与支持网易云API");
            GetType().GetProperty("IsPlaylistSupported", BindingFlags.SetProperty | BindingFlags.Public | BindingFlags.Instance).SetValue(this, true); // Enable Playlist Supporting
        }

        private IDictionary<long, NeteaseLyricInfo> LyricCache { get; } = new Dictionary<long, NeteaseLyricInfo>();

        private List<NeteaseDownloadSongInfo> DownloadSongInfoCache { get; } = new List<NeteaseDownloadSongInfo>();
        ...
        protected override string GetDownloadUrl(SongItem songInfo)
        {
            try
            {
                ...
                NeteaseDownloadSongInfo ds = MainConfig.Instance.LoginSession.LoginStatus ?
                ...
                if (ds != null)
                {
                    ...
                    Log($"由于点歌姬目前只支持播放mp3格式,当前单曲:{string.Join(";", songInfo.Singers)} - {songInfo.SongName} 格式:{ds.Type} 无法播放");
                }
                else
                {
                    Log($"获取下载链接失败(服务器未返回下载链接)");
                }
            }
            catch (WebException e)
            {
                Log($"获取下载链接失败:{e.Message}\r\n这是由于网络原因导致获取失败, 如果多次出现, 请检查你的网络连接。");
            }
            catch (Exception e)
            {
                Log($"获取下载链接失败:{e.Message}");
        ...
        protected override SongInfo Search(string keyword)
        {
            try
            {
                NeteaseSongInfo[] songs = MainConfig.Instance.LoginSession.LoginStatus ?
                    NeteaseMusicApi.SearchSongs(MainConfig.Instance.LoginSession, keyword, 1) :
                    NeteaseMusicApi.SearchSongs(keyword, 1);
                NeteaseSongInfo song = songs.FirstOrDefault();
                if (song?.CanPlay == true)
                {
                    ...
                    {
                        Log($"获取歌词失败了:{e.Message}");
                    }
                    return new SongInfo(this, song.Id.ToString(), song.Name, song.Artists.Select(p => p.Name).ToArray(), lyric?.GetLyricText());
                }
                else
                {
                    Log($"{song.ArtistNames} - {song.Name} : 暂无版权");
                }
            }
            catch (WebException e)
            {
                Log($"搜索单曲失败了:{e.Message}\r\n这是由于网络原因导致搜索失败, 如果多次出现, 请检查你的网络连接。");
            }
            catch (Exception e)
            {
                Log($"搜索单曲失败了:{e.Message}");
        ...
        private NeteaseLyricInfo _GetLyric(long id, bool useCache = true)
        {
            if (!useCache || !LyricCache.ContainsKey(id))
            {
                NeteaseLyricInfo lyric = NeteaseMusicApi.GetLyric(id);
                LyricCache[id] = lyric;
            }
            return LyricCache[id];
        }

        private NeteaseDownloadSongInfo _GetDownloadSongInfo(long id, Quality quality, bool useCache = true)
            => _GetDownloadSongInfo(null, id, quality, useCache);

        private NeteaseDownloadSongInfo _GetDownloadSongInfo(NeteaseSession session, long id, Quality quality, bool useCache = true)
        {
            NeteaseDownloadSongInfo dsi;
            ...
            {
                IDictionary<long, NeteaseDownloadSongInfo> dss = NeteaseMusicApi.GetSongsUrl(session, MainConfig.Instance.Quality, songIds: id);
    ...
    internal static class NeteaseMusicApi
    {
        ...
        /// <param name="offset">偏移量</param>
        public static NeteaseSongInfo[] SearchSongs(string keyWords, int pageSize = 30, int offset = 0)
        ...
        /// <param name="pageSize">本次搜索返回的实例个数上限</param>
        /// <param name="offset">偏移量</param>
        public static NeteaseSongInfo[] SearchSongs(NeteaseSession session, string keyWords, int pageSize = 30, int offset = 0)
        {
            string json = Search(keyWords, SearchType.Song, pageSize, offset);
            JObject j = JObject.Parse(json);
            if (j["code"].ToObject<int>() == 200)
            {
                NeteaseSongInfo[] result = j["result"]["songs"].Select(p => new NeteaseSongInfo(p)).ToArray();
                IDictionary<long, bool> canPlayDic = CheckMusicStatus(session, result.Select(p => p.Id).ToArray());
                foreach (NeteaseSongInfo song in result)
                {
        ...
        public static IDictionary<long, NeteaseDownloadSongInfo> GetSongsUrl(Quality bitRate = Quality.SuperQuality, params long[] songIds)
        ...
        /// <param name="bitRate">比特率上限</param>
        /// <param name="songIds">单曲IDs</param>
        public static IDictionary<long, NeteaseDownloadSongInfo> GetSongsUrl(NeteaseSession session, Quality bitRate = Quality.SuperQuality, params long[] songIds)
        {
            JObject j = _GetPlayerUrl(session, bitRate, songIds);
            return j["data"].ToDictionary(p => p["id"].ToObject<long>(), p => new NeteaseDownloadSongInfo(p["id"].ToObject<long>(), p["br"].ToObject<int>(), bitRate, p["url"].ToString(), p["type"].ToString()));
        }
        ...
        public static NeteaseLyricInfo GetLyric(long songId)
        {
            IDictionary<string, object> data = new Dictionary<string, object>
            {
                ["csrf_token"] = "",
            ...
            if (j["code"].ToObject<int>() == 200)
            {
                ...
                {
                    lyric = new NeteaseLyricInfo(lyricText);
                }
        ...
        public static NeteaseSongInfo[] GetPlayList(long id)
            => GetPlayList(null, id);
        /// <summary>
        /// 获取歌单内的所有单曲
        /// </summary>
        /// <exception cref="NotImplementedException"/>
        /// <param name="id">歌单Id</param>
        public static NeteaseSongInfo[] GetPlayList(NeteaseSession session, long id)
        {
            IDictionary<string, object> data = new Dictionary<string, object>()
            {
                ["id"] = id,
            ...
            switch (code)
            {
                case 200:
                    {
                        NeteaseSongInfo[] result = j["playlist"]["tracks"].Select(p => new NeteaseSongInfo(p)).ToArray();
                        IDictionary<long, bool> canPlayDic = CheckMusicStatus(session, result.Select(p => p.Id).ToArray());
                        foreach (NeteaseSongInfo song in result)
                        {
                            if (canPlayDic.TryGetValue(song.Id, out bool canPlay))
    ...
    internal class NeteaseSession : INotifyPropertyChanged
    {
    ...
    internal class NeteaseSongInfo
    {
        ...
        public NeteaseArtistInfo[] Artists { get; }
        ...
        public NeteaseSongInfo(long id, string name, NeteaseArtistInfo[] artists, NeteaseAlbumInfo album, TimeSpan duration, bool needPaymentToDownload)
        {
            Id = id;
            Name = name;
            Artists = artists;
        ...
        public NeteaseSongInfo(JToken jt) : this(jt["id"].ToObject<long>(), jt["name"].ToString(), (jt["artists"] ?? jt["ar"]).Select(p => new NeteaseArtistInfo(p)).ToArray(), new NeteaseAlbumInfo(jt["album"] ?? jt["al"]), TimeSpan.FromMilliseconds((jt["duration"] ?? jt["dt"]).ToObject<double>()), jt["fee"].ToObject<bool>())
        {

        }
    }

    internal class NeteaseArtistInfo
    {
        public long Id { get; }
        public string Name { get; }
        public NeteaseArtistInfo(long id, string name)
        {
            Id = id;
            Name = name;
        }
        public NeteaseArtistInfo(JToken jt) : this(jt["id"].ToObject<long>(), jt["name"].ToString())
        {

        }
    }

    internal class NeteaseAlbumInfo
    {
        public long Id { get; }
        public string Name { get; }
        public DateTime PublishTime { get; }
        public int Count { get; }
        public NeteaseAlbumInfo(long id, string name, DateTime publishTime, int count)
        {
            Id = id;
            Name = name;
        ...
        public NeteaseAlbumInfo(JToken jt) : this(jt["id"].ToObject<long>(), jt["name"].ToString(), Utils.UnixTimeStamp2DateTime(jt["publishTime"]?.ToObject<long>() ?? 0), jt["size"]?.ToObject<int>() ?? 0)
        {

        }
    }

    internal class NeteaseDownloadSongInfo
    {
        public long Id { get; }
        ...
        public NeteaseDownloadSongInfo(long id, int bitrate, Quality requestQuality, string url, string type)
        {
            Id = id;
            Bitrate = bitrate;
        ...
        public NeteaseDownloadSongInfo(JToken jt, Quality requestQuality) : this(jt["id"].ToObject<long>(), jt["br"].ToObject<int>(), requestQuality, jt["url"].ToString(), jt["type"].ToString())
        {

        }
    }

    internal class NeteaseLyricInfo
    {
        ...
        public NeteaseLyricInfo()
        {
            LrcWord = new SortedDictionary<double, string>();
        }

        public NeteaseLyricInfo(string lyricText) : this()
            => AppendLrc(lyricText);
